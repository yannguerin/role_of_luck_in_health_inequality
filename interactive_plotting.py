...
def model_mortality_from_population(population):
    ...
    model_mortality = model_mortality.reset_index(name="count").rename(
        columns={"age_of_death": "age"}
    )
    model_mortality = (
        model_mortality.sort_values("age", ascending=False)
        .set_index("age")
        .cumsum()
        .iloc[::-1]
    )
    model_mortality = model_mortality.rename(
        columns={"count": "Model survivors at age x"}
    ).reset_index()
    return model_mortality


def interactive_plot_of_mortality(
    models, sim_num_to_description, defaults: list | None = None
):
    ...
    mortality["Model survivors at age x"] = (
        mortality["Actual survivors at age x"] / (100_000 / 10_000)
    ).astype(int)
    ...
    default_sims_to_show = [
        simulation_num_to_description_mapping[sim_num] for sim_num in defaults
    ]
    ...
    fig = px.line(
        all_sims_mortality,
        x="age",
        y="Model survivors at age x",
        color="simulation",
        title="Mortality for All Simulations",
    )

    fig.for_each_trace(
        lambda trace: (
            trace.update(visible="legendonly")
            if trace.name not in default_sims_to_show
            else ()
        )
    )
...
def plot_inequity_mortality_curves(
    lvc_model, c_or_e_or_ha: str, index=None, number_of_years=80, population_size=10_000
):
    ...
    if c_or_e_or_ha == "c":
        min_pop = population[
            population.circumstance_score == population.circumstance_score.min()
        ]
        max_pop = population[
            population.circumstance_score == population.circumstance_score.max()
        ]
        mid_pop = population[
            population.circumstance_score == population.circumstance_score.median()
        ]
    elif c_or_e_or_ha == "e":
        population["effort_quantiles"] = pd.qcut(
            population.effort_score, q=[0, 0.1, 0.45, 0.55, 0.9, 1.0]
        )
        min_pop = population[
            population.effort_quantiles == population.effort_quantiles.min()
        ]
        max_pop = population[
            population.effort_quantiles == population.effort_quantiles.max()
        ]
        mid_pop = population[
            population.effort_quantiles
            == sorted(population.effort_quantiles.unique())[2]
        ]
    elif c_or_e_or_ha == "ha":
        population["health_ability_quantiles"] = pd.qcut(
            population.health_ability, q=[0, 0.1, 0.45, 0.55, 0.9, 1.0]
        )
        min_pop = population[
            population.health_ability_quantiles
            == population.health_ability_quantiles.min()
        ]
        max_pop = population[
            population.health_ability_quantiles
            == population.health_ability_quantiles.max()
        ]
        mid_pop = population[
            population.health_ability_quantiles
            == sorted(population.health_ability_quantiles.unique())[2]
        ]
    else:
    ...
    mortality = mortality.rename(
        columns={"Actual survivors at age x": "Model survivors at age x"}
    )
    mortality["Model survivors at age x"] = (
        mortality["Model survivors at age x"] / (100_000 / population_size)
    ).astype(int) / population_size
    mortality["segment"] = "real_world"

    # Dataframe for all subpops
    all_model_mortalities = pd.DataFrame()

    for sub_pop, sub_pop_name in zip(
        (population, min_pop, max_pop, mid_pop), ("full", "min", "max", "median")
    ):
    ...
    fig = px.line(
        all_model_mortalities,
        x="age",
        y="Model survivors at age x",
        color="segment",
        title=f"Mortality Curves for varying levels of {c_or_e_or_ha.upper()}",
    )
...
def distribution_of_age_of_death_stacked_by_number_of_health_shocks(
    lvc_model, index=None, include_neonatal_deaths=False
):
    if not index is None:
        shocks_taken_data = {
            k: v for k, v in lvc_model.shocks_taken_data.items() if k in index
        }
    ...
    if include_neonatal_deaths:
        return px.histogram(
            population[(~population.alive) & (population.age_of_death > 0)],
            x="age_of_death",
            color="number_of_health_shocks",
            color_discrete_sequence=px.colors.sequential.Magenta,
            nbins=int(population.age_of_death.max()),
            title="Age of Death by Number of Health Shocks",
        )
...
def plot_distribution_of_scores(lvc_model, index=None):
    ...
    fig.add_trace(
        go.Histogram(x=population.circumstance_score, name="Circumstance"), row=1, col=1
    )
    fig.add_trace(go.Histogram(x=population.effort_score, name="Effort"), row=1, col=2)
    fig.add_trace(
        go.Histogram(x=population.health_ability, name="Health Ability"), row=2, col=1
    )
    fig.add_trace(
        go.Histogram(x=population.health_score, name="Health Score"), row=2, col=2
    )
...
def plot_individuals_in_the_population_from_model(lvc_model, index=None):
    ...
    if not index is None:
        ...
        pop_index = random.sample(
            list(
                lvc_model.population_over_time[
                    lvc_model.population_over_time.age_of_death != 0
                ].index.unique()
            ),
            12,
        )
    ...
    subplot_titles = [
        f"Circumstance {population[population.index == i].head(1)['circumstance_score'].iloc[0]:.2f} | Effort {population[population.index == i].head(1)['effort_score'].iloc[0]:.2f} | Health Ability {population[population.index == i].head(1)['health_ability'].iloc[0]:.2f}"
    ...
    for i, index in enumerate(individuals):
        ...
        fig.add_trace(
            go.Scatter(
                x=single_individual["time"],
                y=single_individual["unshocked_health_score"],
                marker={"color": "grey"},
                name=index,
                mode="lines",
                opacity=0.5,
        ...
        fig.add_trace(
            go.Scatter(
                x=single_individual["time"],
                y=single_individual["health_score"],
                marker={"color": "black"},
                name=index,
                mode="lines",
                showlegend=False,
        ...
        for ix, row in single_individual[
            single_individual.encountered_shock
        ].iterrows():
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name=row["shock_causes"],
                    line=dict(
                        color=shock_cause_colors[
                            shock_causes.index(row["shock_causes"])
                        ],
                        width=1,
                        dash=ls,
                    ),
...
def plot_shock_magnitudes(lvc_model):
    ...
    for shock in shock_data.values():
        for shock_cause, shock_magnitude in zip(
            shock["shocks"], shock["shock_magnitudes"]
        ):
            shock_magnitudes[shock_cause].extend(
                [1 - magn for magn in shock["shock_magnitudes"]]
            )

    shock_magnitudes_df = pd.DataFrame(
        dict([(k, pd.Series(v)) for k, v in shock_magnitudes.items()])
    )
    shock_magnitudes_df = shock_magnitudes_df[
        [
            "Total cancers",
            "Ischemic heart disease",
            "Diabetes mellitus",
    ...
    return px.histogram(
        shock_magnitudes_df,
        title="Shock Magnitudes by Shock Cause",
        labels={"count": "Number of Shocks with Magnitude", "value": "Shock Magnitude"},
    )
...
def plot_age_of_health_shocks(lvc_model):
    ...
    age_of_health_shock_df["encountered_shock"] = pop_over_time.groupby(
        "time"
    ).encountered_shock.sum()
    age_of_health_shock_df["taken_shock"] = pop_over_time.groupby(
        "time"
    ).taken_shock.sum()
    age_of_health_shock_df["time"] = list(range(80))

    return px.bar(
        age_of_health_shock_df,
        y=["encountered_shock", "taken_shock"],
        x="time",
        title="Number of Health Shocks by Age",
        labels={"value": "Number of Health Shocks", "time": "age"},
    )
...
def interactive_lorenz_curve_plot(models, sim_num_to_description, alive_only=False):
    ...
    for sim_num, model in models.items():
        ...
        population.loc[
            population[
                (population.health_score < 0) | (population.health_score == 1)
            ].index,
            "health_score",
        ] = 0
    ...
    fig.update_layout(
        title=title, xaxis_title="Percentiles (p)", yaxis_title="L(p)", showlegend=True
    )
...
def interactive_cross_sectional_lorenz_curve_plot(
    cross_sectional_dfs, sim_num_to_description, alive_only=False
):
    fig = go.Figure()
...
def cross_sectional_health_score(model):
    ...
    for time_bin, pop in pop_over_time[pop_over_time.alive == True].groupby("time_bin"):
        ...
        cross_sectional_population = pd.concat(
            [
                cross_sectional_population,
                pop[pop.time == time].sample(
                    n=int(10_000 * bins_to_proportion[time_bin]), replace=True
                ),
            ]
        )
...
def build_cross_sectional_datasets(
    models: dict, alive_only: bool = True
    ...
    for sim_num, model in models.items():
        pop_over_time = model.population_over_time.copy()
        # setting the time bin for each time in the population over time df
        pop_over_time["time_bin"] = pop_over_time.time.apply(
            lambda x: bins[int(x // 5)]
        )
        ...
        for time_bin, pop in groups:
            time = int(
                time_bin.replace(" to ", " ").replace(" years", " ").split(" ")[1]
            )
            # sampling the number of agents in the time bin equal to the proportion to ensure there are a total of 10_000 agents
            cross_sectional_population = pd.concat(
                [
...
def interactive_cross_sectional_health_scores_plot(
    cross_sectional_dfs, simulation_num_to_description_mapping
):
    subplot_titles = [
        simulation_num_to_description_mapping[sim_num]
        for sim_num in list(cross_sectional_dfs.keys())
    ]

    fig_shape = (2, 3)

    fig = make_subplots(*fig_shape, subplot_titles=subplot_titles)
    for i, (sim_num, cross_sectional_population) in enumerate(
        cross_sectional_dfs.items()
    ):
        ...
        fig.add_trace(
            go.Histogram(
                x=cross_sectional_population.health_score,
                name=simulation_num_to_description_mapping[sim_num],
                nbinsx=50,
            ),
            row=row,
            col=col,
...
def interative_health_scores_plot(models, simulation_num_to_description_mapping):
    subplot_titles = [
        simulation_num_to_description_mapping[sim_num]
        for sim_num in list(models.keys())
    ]
    ...
    for i, (sim_num, model) in enumerate(models.items()):
        ...
        population.loc[
            population[
                (population.health_score < 0)
                | (population.health_score == 1)
                | (~population.alive)
            ].index,
            "health_score",
        ] = 0
...
def interative_health_scores_plot_by_age(
    models, simulation_num_to_description_mapping, age
):
    subplot_titles = [
        simulation_num_to_description_mapping[sim_num]
        for sim_num in list(models.keys())
...
def interactive_plot_of_health_score_distributions_over_time(models):
    ...
    population_over_time.loc[
        population_over_time[
            (population_over_time.health_score < 0)
            | (population_over_time.health_score == 1)
            | (~population_over_time.alive)
        ].index,
        "health_score",
    ] = 0
...
def plot_proportion_of_deaths_by_circumstance_score(models):
    ...
    for sim_num in models.keys():
        if sim_num == "Pure luck model":
            ...
            canada_education, _, _ = params.parse_empirical_data(
                "./luck_vs_circumstance/datasets"
            )
            c_dist = dist.Circumstance_Distribution(
                canada_education["Circumstance_Score"], canada_education["Proportion"]
            )
        ...
        population = population[
            ~population.cause_of_death.isin(["accidental", "neonatal"])
        ]
        temp = (
            population.groupby("circumstance_score")
            .age_of_death.value_counts(True)
            .to_frame()
            .reset_index()
        )
    ...
    fig = px.line(
        prop_frame,
        x="circumstance_score",
        y="proportion",
        color="simulation",
        title="Proportion of Agents surviving to age 80 by Circumstance",
    )
...
def plot_proportion_of_deaths_by_health_ability_decile(models):
    ...
    for sim_num in models.keys():
        ...
        temp = (
            population.groupby("ha_decile_2")
            .age_of_death.value_counts(True)
            .to_frame()
            .reset_index()
        )
        temp["health_ability_decile_index"] = temp.ha_decile_2.apply(
            lambda x: sorted(list(temp.ha_decile_2.unique())).index(x)
        )
    ...
    fig = px.line(
        prop_frame,
        x="health_ability_decile_index",
        y="proportion",
        color="simulation",
        title="Proportion of Agents surviving to age 80 by Health Ability Decile",
    )
